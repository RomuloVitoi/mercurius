...
function makeResolver ({ service, createOperation, transformData, isQuery, isReference, isSubscription, typeToServiceMap, serviceMap, name }) {
  return async function (parent, args, context, info) {
    ...
    const { reply, __currentQuery, lruGatewayResolvers, pubsub, entityResolversFactory } = context
    ...
    if (isSubscription) {
      ...
      context.subscriptionMap.set(context.subscriptionId, { service, subscriptionId })
      return pubsub.subscribe(`${service.name}_${subscriptionId}`)
    }

    const entityResolvers = reply
      ? (reply.entityResolversFactory ? reply.entityResolversFactory.create() : reply[kEntityResolvers])
      : entityResolversFactory.create()
